...
class LogisticRegressor:
    ...
    def fit(
        ...
        for i in range(num_iterations):  # Fill the None here
            ...
            dw = (1/m)*np.transpose(X)@(y_hat-y) # Derivative w.r.t. the coefficients
    ...
    def lasso_regularization(self, dw, m, C):
        ...
        for peso in pesos:
            if peso > 0:
                resultado.append((C/m)*1)
            elif peso < 0:
                resultado.append((C/m)*(-1))
            else:
                resultado.append(0)
                
        lasso_gradient = np.array(resultado)
    ...
    def ridge_regularization(self, dw, m, C):
        ...
        ridge_gradient = np.array([(C/m)*peso for peso in self.weights])
